...
export class ChatService {
   ...
   */
  async sendMessage(sessionId: string, text: string, metadata?: Record<string, any>): Promise<SendMessageResponse | null> {
    try {
  ...
  async getSessionMessages(sessionId: string, limit: number = 50, offset: number = 0): Promise<ChatMessage[]> {
    try {
      const response = await makeAuthenticatedRequest<ChatMessage[]>("GET", `/chat/sessions/${sessionId}/messages?limit=${limit}&offset=${offset}`);
      return response;
    } catch (error: any) {
